...
# Steganography App Banner
print("#### Steganography App ####")

# Ask user for input
user_input = input("Enter 'e' to encode text in an image, 'd' to decode text from an image: ")

# Create a new MipsTER instance
m = MipsTER()

if user_input == 'e':
    # Assemble the encode assembly code
    m.assemble("""
               .data
    startProgram:		.asciiz    	 	"The program for hidding sentence into a BMP image has began... \n"
    menuFilename:		.asciiz     	"Enter a number for choosing the image BMP to encode: 1 => sample.bmp, 2 => sample2.bmp: "
    imageOne:			.asciiz     	"C:/Users/HP/OneDrive/Steganography_Project/ImageSource/sample.bmp"
    imageTwo:   		.asciiz     	"C:/Users/HP/OneDrive/Steganography_Project/ImageSource/sample2.bmp"
...
main:
	...
	jal stringToArray #go to the function string to array
	...
	move $a2, $s4 #load the value to read into $a2
	...
	move $s6, $v1 #save the return value of te string for the filename into $s6
...
changePixelImg: ###### Function for changing the pixel of the image
	...
	WHILEBITARR:
		...
		# The parenthesis around $s5 indicate that it is the address of the byte to be stored, not the content of the register $s5
		sb $t4, 0($s5) #add the new value to the pixel array
	...
	ENDBITARR:
		...
		lw $t6, endString($t8) #value of the arraybit($t1)
		add $t4, $t4, 0 #add the value of the pixel to $t4
...
makingFileArray: ####### Function for making the array of the file image
	...
	move $a0, $s4 #size to alloc in $a0
	...
	# to get the total length of the file 
	# used to determine when to stop reading the image data
...
# to read the contents of a file and store it in memory
...
arrayCharToArrayBit:
    ...
    li $t0, 0 #counter for iterating through array of chars
    li $t1, 7 #counter for iterating through bits of each char
    li $t2, 0 #variable to store address of current bit in array of bits
    li $t3, 0 #variable to store value of current bit
    li $t4, 0 #variable to store address of current char in array

    move $t4, $s1 #move address of char array into $t4

    WHILEARRAYBIT:
        beq $t0, $s2, ENDARRAYBIT #exit loop if all chars have been processed

        lb $t5, 0($t4) #load current char into $t5
	...
	jal printString #go to the function for print a string
        ...
        WHILELOOPBIT:
            blt $t1, $zero, ENDWHILEARRAYBIT #exit loop if all bits of current char have been processed

            srlv $t3, $t5, $t1 #shift right to get value of current bit
            andi $t3, $t3, 1 #and with 1 to isolate current bit
            sb $t3, arrayBit($t2) #store bit in array of bits

            add $t2, $t2, 1 #increment address of current bit in array of bits
            sub $t1, $t1, 1 #decrement bit counter
        ...
        ENDWHILEARRAYBIT:
            ...
            addi $t4, $t4, 1 #increment address of current char in array
            li $t1, 7 #reset bit counter for next char
            add $t0, $t0, 1 #increment char counter
            j WHILEARRAYBIT
    ENDARRAYBIT:
...
mallocBit: ######Function to malloc bit
	...
	# used when the program needs to store data whose size is not known until runtime
	li $v0, 9 #system call for alloc 
	syscall #exec the cmd
...
######################################################################################
    ...
    # Assemble the decode assembly code
    m.assemble("""
        # Assembly code to decode text from an image
    """)
    print("Decoding text from image...")
else:
...
# Inform user that the code has been executed successfully
print("Mips assembly code executed successfully.")
